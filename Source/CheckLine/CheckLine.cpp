...
using namespace std;
int CheckLine::CheckInput(char* s, int Len) {
    int newLen = Len;
    int ms = 0; bool f;
    int shift = 0;
...
void CheckLine::CheckModul(char* newStr, char* prevStr, int& prevPos, int& newPos, int& shift) {
    bool  f = 0;
    if (prevPos == 0) {
        newStr[newPos] = 'm';
        newStr[newPos + 1] = '(';
...
void CheckLine::CreateNewStr(char* newStr, char* prevStr, int prevLen) {
    int  shift = 0, space = 0;
    if (prevStr[0] == '-') {
        newStr[0] = '0';
    ...
    for (int i = 0; i < prevLen; i++) {
        ...
        if (i > 0 && prevStr[prevPos - 1] == '(' && prevStr[prevPos] == '-') {
            ...
            CheckLine::CheckModul(newStr, prevStr, prevPos, newPos, shift);
        }
        else {
...
bool CheckLine::CheckSymbol(char* s, int i) {
    char allSymbols[] = "+-*/^m ()|";
    bool f = 1;
...
void CheckLine::CoutShift(char* s, int i, int& newLen) {
    if (i > 0 && s[i - 1] == '(' and s[i] == '-') {
        newLen += 1;
    }
    else if (s[i] == '|') {}
